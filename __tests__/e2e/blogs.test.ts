...
const auth = { Authorization: "Basic YWRtaW46cXdlcnR5" };
const dateISORegEx = expect.stringMatching(
  /\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d\.\d+([+-][0-2]\d:[0-5]\d|Z)/
);
const anyString = expect.any(String);

const getErrorsMessages = <T>(...fields: (keyof T)[]) => ({
...
const getOverMaxLength = (max: number) => "a".repeat(max + 1);

const sortByField = <T = any[]>(
  arr: T[],
  field: keyof T,
...
describe("Test blogs api", () => {
  ...
  it("Create blog. Incorrect body cases. Should return 400 and errorsMessages", async () => {
    ...
    const secondRes = await request(app)
      .post(blogs_router)
      .set(auth)
      .send({ name: getOverMaxLength(15), youtubeUrl: getOverMaxLength(100) });
    ...
    expect(thirdRes.body).toEqual(getErrorsMessages<ReqBodyBlog>("youtubeUrl"));
