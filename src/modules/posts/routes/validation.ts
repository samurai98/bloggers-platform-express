...
const titleValidation = body('title').trim().notEmpty().isLength({ max: 30 }).withMessage('Title length error');
...
const contentValidation = body('content').trim().notEmpty().isLength({ max: 1000 }).withMessage('Content length error');
