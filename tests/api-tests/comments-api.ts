...
import { comments_router, delete_all_router } from "../../src/routers";
  ...
  describe("Test comments api", () => {
    ...
    it("Update comment. Incorrect body cases. Should return 400 and errorsMessages", async () => {
      const firstRes = await request(app)
        .put(`${comments_router}/${createdComment.id}`)
        .set(bearerAuth)
        .send();
    ...
    it("Delete comment. Should delete comment and return 204", async () => {
      await request(app)
        .delete(`${comments_router}/${createdComment.id}`)
        .set(bearerAuth)
        .expect(HTTP_STATUSES.NO_CONTENT_204);
