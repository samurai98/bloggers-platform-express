...
export const testDevicesApi = () =>
  describe('Test securityDevices api', () => {
    ...
    it('Get devices by id. Should return 200 and active sessions', async () => {
      const res = await request(app).get(router.securityDevices).set(bearerAuth);

      expect(res.statusCode).toEqual(HTTP_STATUSES.OK_200);
      expect(res.body).toEqual(
        expect.arrayContaining([{ ip: expectedIp, deviceId: anyString, title: anyString, lastActiveDate: anyString }])
      );
    ...
    it('Delete all devices exclude current. Should return 204', async () => {
      await request(app).delete(router.securityDevices).set(bearerAuth).expect(HTTP_STATUSES.NO_CONTENT_204);

      await request(app).get(router.securityDevices).set(bearerAuth).expect(HTTP_STATUSES.OK_200, [createdSession[3]]);
    });
